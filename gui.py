...
def main() -> int:
    ...
    try:
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("MechaLit.App")
    except Exception:
        pass
    ...
    # 打包/起窗自检（构建与桥探针用）：环境变量 MECHA_SELFTEST=1 时 3 秒后自动退出，
    # 退出码 0 = 整个界面能正常拉起。自检模式不弹任何模态引导。
    if os.environ.get("MECHA_SELFTEST") == "1":
