...
## 1. 分级（PI 2026-07-18 终审：S1 / R2 / C1）

> **升级记录**：初判 R1（2026-07-18 上午）→ **PI 同日拍板升 R2**，采纳此前已暴露的 R2 论点。

| 轴 | 等级 | 判级三问与依据 |
|---|---|---|
| 规模 S | **S1** | 个人长期使用的单机桌面工具（PI 自用采集 GUI） |
| 风险 R | **R2** | ①不可逆动作？导入可逆（Zotero 回收站）但**批量污染清理成本高**②真实数据与密钥？有——Zotero API **写凭证** + PI 核心科研文献库（重要数据资产）③对外？否。文献库属重要数据、持写凭证 → R2（可能造成重要数据损失口径）。PI 2026-07-18 拍板 |
| 商业 C | **C1** | 项目所有者自己长期使用，不对外交付/收费 |

### R2 义务落点（母法 4.8 R2；机器版见 policy.yaml R2 段）

- **独立复核**：每候选版本一轮隔离上下文复核（不继承施工结论；条件允许异构模型）——2026-07-18 对抗轮即首次实践
- **对抗性测试（7.5）**：每候选版本一轮、由未参与施工的独立上下文设计——`tests/adversarial_test.py` 常驻 CI 每推跑 + 每候选版本追加新变体
- **变异测试（7.7）**：每发布周期一轮核心模块（lit/ 数据层），杀灭率 <70% 逐个分析存活变异体——首轮排下个发布周期（BL-08）
- **故障注入**：smoke 失败注入（超时/断网/非零退出/坏输出）+ adversarial 畸形注入，CI 每推跑
- **事故响应（9.2）**：发现→限损→留证→恢复→通知判断→根因→护栏回流七步；应急预授权见 §5
- **迁移和回滚演练**：不适用——无自有 Schema/数据库（一句话理由，母法 2.4）
- **需求追踪（6.2）**：轻量自动化——INV/VS/BL/EX 编号贯穿 commit message、测试断言与五件套清单，git 历史即追踪链
