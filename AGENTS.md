...
- **项目合同**：[PROJECT-CONTRACT.md](./PROJECT-CONTRACT.md)（分级 S1/R1/C1 / 预算 / 预授权 / 测试缺省值）
...
R1 项目：写审分离必守（施工方自报不作数，主驾新眼睛独立跑验证）；每候选版本对抗测试非强制（R2+ 义务），
重大改动（引擎桥 / 导入路径 / 凭证面）建议一轮异构复核。分支保护为例外 EX-01，由「CI 每推必跑 +
红灯阻断 + 单执行体不 force-push main」补偿。
